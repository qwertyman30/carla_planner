    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                # --------------------------------------------------------------
                circle_locations[:, 0] = path[0][j] + [offset * int(np.cos(path[2][j])) for offset in self._circle_offsets]
                circle_locations[:, 1] = path[1][j] + [offset * int(np.sin(path[2][j])) for offset in self._circle_offsets]
                # --------------------------------------------------------------
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                score = np.sqrt((paths[i][1][-1] - goal_state[1]) ** 2 + (paths[i][0][-1] - goal_state[0]) ** 2)
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                            # --------------------------------------------------
                            score += self._weight * np.sqrt(
                                (paths[i][1][-1] - paths[j][1][-1]) ** 2 + (paths[i][0][-1] - paths[j][0][-1]) ** 2)
                            # --------------------------------------------------
