...
class BehaviouralPlanner:
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        (including closest_len) that is greater than or equal to self._lookahead.
        ...
        while wp_index < len(waypoints) - 1:
            curr_waypoint, next_waypoint = waypoints[wp_index], waypoints[wp_index + 1]
            arc_length += math.sqrt(
                (next_waypoint[0] - curr_waypoint[0]) ** 2 + (next_waypoint[1] - curr_waypoint[1]) ** 2)
            if arc_length >= self._lookahead:
                return wp_index + 1
            wp_index += 1
        # ------------------------------------------------------------------
...
def get_closest_index(waypoints, ego_state):
    ...
    for i, waypoint in enumerate(waypoints):
        dist = math.sqrt((ego_state[0] - waypoint[0]) ** 2 + (ego_state[1] - waypoint[1]) ** 2)
        if dist <= closest_len:
            closest_index = i
            closest_len = dist
    # ------------------------------------------------------------------
